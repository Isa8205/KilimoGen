...
export function AddHarvestModal({ seasonId, onClose }: AddHarvestModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await window.electron.invoke("add-harvest", data)
      ...
      notify(false, "Failed to add harvest")
    } finally {
      setSending(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      ...
      <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-lg">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-semibold text-accent">Record New Harvest</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-primary mb-1">Harvest Name</label>
            <input
              type="text"
              name="name"
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
              placeholder="e.g., Harvest 1"
              required
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-primary mb-1">Target (kg)</label>
            <input
              type="number"
              name="target"
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
